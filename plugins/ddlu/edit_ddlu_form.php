...
require_once($CFG->dirroot . '/question/type/ddlu/edit_ddlu_form_base.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlu_edit_form extends qtype_ddlu_edit_form_base {
    ...
    public function data_preprocessing($question) {
        ...
        file_prepare_draft_area($draftitemid, $this->context->id, 'qtype_ddlu',
    ...
    public function js_call() {
        ...
        $PAGE->requires->js_call_amd('qtype_ddlu/form', 'init');
    }
    ...
    protected function definition_draggable_items($mform, $itemrepeatsatstart) {
        $mform->addElement('header', 'draggableitemheader',
                                get_string('draggableitems', 'qtype_ddlu'));
    ...
    protected function draggable_item($mform) {
        ...
        $grouparray = array();
        $dragitemtypes = array('image' => get_string('draggableimage', 'qtype_ddlu'));
        $grouparray[] = $mform->createElement('select', 'dragitemtype',
                                            get_string('draggableitemtype', 'qtype_ddlu'),
                                            $dragitemtypes,
    ...
    protected function drop_zone($mform, $imagerepeats) {
        ...
        $grouparray[] = $mform->createElement('select', 'choice',
                                    get_string('draggableitem', 'qtype_ddlu'), $options);
        ...
        $dropzone = $mform->createElement('group', 'drops',
                get_string('dropzone', 'qtype_ddlu', '{no}'), $grouparray);
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; $i < $data['nodropzone']; $i++) {
            ...
            if ($imagechoicepresent) {
                if (!$ytoppresent) {
                    $errors["drops[$i]"] = get_string('formerror_noytop', 'qtype_ddlu');
                } else if (!$ytopisint) {
                    $errors["drops[$i]"] = get_string('formerror_notintytop', 'qtype_ddlu');
                }
                if (!$xleftpresent) {
                    $errors["drops[$i]"] = get_string('formerror_noxleft', 'qtype_ddlu');
                } else if (!$xleftisint) {
                    $errors["drops[$i]"] = get_string('formerror_notintxleft', 'qtype_ddlu');
                }

                if ($data['drags'][$choice - 1]['dragitemtype'] != 'word' &&
                                        !self::file_uploaded($data['dragitem'][$choice - 1])) {
                    $errors['dragitem['.($choice - 1).']'] =
                                    get_string('formerror_nofile', 'qtype_ddlu', $i);
                }

                //if (isset($allchoices[$choice]) && !$data['drags'][$choice - 1]['infinite']) {
                 //   $errors["drops[$i]"] =
                //            get_string('formerror_multipledraginstance', 'qtype_ddlu', $choice);
                //    $errors['drops['.($allchoices[$choice]).']'] =
                //            get_string('formerror_multipledraginstance', 'qtype_ddlu', $choice);
               //     $errors['drags['.($choice - 1).']'] =
               //             get_string('formerror_multipledraginstance2', 'qtype_ddlu', $choice);
               // }
                $allchoices[$choice] = $i;
            } else {
                if ($ytoppresent || $xleftpresent || $labelpresent) {
                    $errors["drops[$i]"] =
                            get_string('formerror_noimageselected', 'qtype_ddlu');
                }
            }
        }
        for ($dragindex = 0; $dragindex < $data['noitems']; $dragindex++) {
            ...
            if ($data['drags'][$dragindex]['dragitemtype'] == 'word') {
                ...
                $errormessage = get_string('formerror_disallowedtags', 'qtype_ddlu', s($allowedtags));
            } else {
                $allowedtags = '';
                $errormessage = get_string('formerror_noallowedtags', 'qtype_ddlu');
            }
