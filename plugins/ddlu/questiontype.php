...
require_once($CFG->dirroot . '/question/type/ddlu/questiontypebase.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_ddlu extends qtype_ddlu_base {

    protected function make_choice($dragdata) {
        return new qtype_ddlu_drag_item($dragdata->label, $dragdata->no,
                                        $dragdata->draggroup, /*$dragdata->infinite,*/ $dragdata->id);
    }

    protected function make_place($dropzonedata) {
        return new qtype_ddlu_drop_zone($dropzonedata->label, $dropzonedata->no,
                                                    $dropzonedata->group,
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_ddlu', array('questionid' => $formdata->id));
        if (!$options) {
            ...
            $options->id = $DB->insert_record('qtype_ddlu', $options);
        }
        ...
        $DB->update_record('qtype_ddlu', $options);
        $DB->delete_records('qtype_ddlu_drops', array('questionid' => $formdata->id));
        foreach (array_keys($formdata->drops) as $dropno) {
            ...
            $DB->insert_record('qtype_ddlu_drops', $drop);
        }

        // An array of drag no -> drag id.
        $olddragids = $DB->get_records_menu('qtype_ddlu_drags',
        ...
        foreach (array_keys($formdata->drags) as $dragno) {
            ...
            if ($info['filecount'] > 0 || (trim($formdata->draglabel[$dragno]) != '')) {
                ...
                if (isset($olddragids[$dragno + 1])) {
                    ...
                    $DB->update_record('qtype_ddlu_drags', $drag);
                } else {
                    $drag->id = $DB->insert_record('qtype_ddlu_drags', $drag);
                }

                if ($formdata->drags[$dragno]['dragitemtype'] == 'image') {
                    file_save_draft_area_files($draftitemid, $formdata->context->id,
                                        'qtype_ddlu', 'dragimage', $drag->id,
                                        array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
                } else {
                    // Delete any existing files for draggable text item type.
                    $fs = get_file_storage();
                    $fs->delete_area_files($formdata->context->id, 'qtype_ddlu',
                                                                'dragimage', $drag->id);
                }
        ...
        if (!empty($olddragids)) {
            ...
            $DB->delete_records_select('qtype_ddlu_drags', "id $sql", $params);
        }
        file_save_draft_area_files($formdata->bgimage, $formdata->context->id,
                                    'qtype_ddlu', 'bgimage', $formdata->id,
                                    array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
    }
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_ddlu', 'bgimage', $questionid);
        $dragids = $DB->get_records_menu('qtype_ddlu_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->move_area_files_to_new_context($oldcontextid,
                                    $newcontextid, 'qtype_ddlu', 'dragimage', $dragid);
        }
    ...
    protected function delete_files($questionid, $contextid) {
        ...
        $dragids = $DB->get_records_menu('qtype_ddlu_drags',
                                                array('questionid' => $questionid), 'id', 'id,1');
        foreach ($dragids as $dragid => $notused) {
            $fs->delete_area_files($contextid, 'qtype_ddlu', 'dragimage', $dragid);
        }
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $output .= $format->write_combined_feedback($question->options,
                                                    $question->id,
                                                    $question->contextid);
        $files = $fs->get_area_files($contextid, 'qtype_ddlu', 'bgimage', $question->id);
        $output .= "    ".$this->write_files($files, 2)."\n";;

        foreach ($question->options->drags as $drag) {
            $files =
                    $fs->get_area_files($contextid, 'qtype_ddlu', 'dragimage', $drag->id);
            $output .= "    <drag>\n";
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra=null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'ddlu') {
            return false;
        }
